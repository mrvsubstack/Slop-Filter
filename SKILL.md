...
description: >
  ...
  shares a paragraph and says anything like "too formal," "too robotic," "fix the voice,"
...
3. Rewrite — clean version, no explanation

If the user only asks for a rewrite, skip to step 3.
...
Read `phrases.md` and `structures.md` for every scoring or rewriting task. Read `examples.md` when the user asks to see examples or when you need calibration on a subtle case.
...
**Multi-pass rule:** After any rewrite, run the Quick Checks again before scoring. A first pass clears the obvious patterns. The second pass catches what survived. Deliver only after the second check clears.
