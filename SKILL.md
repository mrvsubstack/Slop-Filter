...
**Scoring is opt-in.** Only run the scoring rubric if the user explicitly asks for a score, asks "is this clean enough to publish?", or asks "how far off is this?" "Is this correct?" " is this clear?". Do not lead with a number by default — the diagnosis already contains everything the score summarizes, with more precision.
